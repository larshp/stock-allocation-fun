...
| -------------------- | -------------------------------------------------------- |
| `ZSTOCK_ALLOC_PLTS`  | how every plant you may see stands, and can be mailed when one of them is short or did not run: materials, short lines, how much is short, the oldest day anything is still waiting for, and whether a run has touched it today |
...
| `ZSTOCK_ALLOC_TRF`   | transfers somebody has proposed between plants, the day each is needed by, and whether they were raised, dropped or are still waiting for an answer |
