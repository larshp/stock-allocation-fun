...
CLASS ltcl_allocation_audit_sap DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS records_preview_provenance FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS filters_preview_runs FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_invalid_preview_flag FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_invalid_preview_filter FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_bad_rejection_metric FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_bad_final_counts FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_success_message FOR TESTING
      RAISING zcx_stock_allocation.
    ...
    METHODS rejects_inverted_timestamp FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_invalid_clock_time FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS filters_duration_bounds FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_malformed_filter_dates FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_bad_purge_time FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_bad_purge_horizon FOR TESTING
      RAISING zcx_stock_allocation.
    ...
    METHODS latest_summary_tie_breaker FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS latest_success_streak FOR TESTING
      RAISING zcx_stock_allocation.
    ...
    METHODS rejects_invalid_purge_status FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS filters_purge_preview_runs FOR TESTING
      RAISING zcx_stock_allocation.
    ...
    METHODS rejects_rejection_commit FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_default_commit_returns FOR TESTING
      RAISING zcx_stock_allocation.
...
CLASS ltcl_allocation_audit_sap IMPLEMENTATION.
  METHOD records_preview_provenance.
    ...
    lv_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-PREVIEW'
      iv_plant            = '1000'
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = 1 ).
    cl_abap_unit_assert=>assert_equals(
      act = lt_runs[ 1 ]-preview
      exp = abap_true ).
    DELETE FROM zstockalloc_run
      WHERE run_id = @lv_run_id.
  ENDMETHOD.

  METHOD rejects_invalid_preview_flag.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    ...
    TRY.
        lo_cut->start_run(
          iv_material         = 'MATERIAL-AUDIT-BAD-PREVIEW'
          iv_plant            = '1000'
          iv_storage_location = '0001'
          iv_unit             = 'EA'
          iv_available        = 1
      ...
      CATCH zcx_stock_allocation INTO DATA(lo_error).
        ...
        cl_abap_unit_assert=>assert_equals(
          ...
          exp = 'Audit preview flag is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
    CLEAR lv_raised.
    TRY.
        lo_cut->record_rejection(
          iv_material         = 'MATERIAL-AUDIT-BAD-REJECTION-PREVIEW'
          iv_plant            = '1000'
          iv_storage_location = '0001'
          iv_unit             = 'EA'
          iv_available        = 0
          iv_message          = 'Invalid rejection preview'
          iv_preview          = 'Y' ).
  ...
  METHOD filters_preview_runs.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA lv_preview_run_id TYPE zif_allocation_audit=>ty_run_id.
    DATA lv_operational_run_id TYPE zif_allocation_audit=>ty_run_id.
    DATA lt_runs TYPE zif_allocation_audit=>tt_runs.

    CREATE OBJECT lo_cut TYPE zcl_allocation_audit_sap.
    lv_preview_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-PREVIEW-FILTER'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
      iv_available        = 5
    ...
    lo_cut->finish_run(
      ...
      iv_full_count = 1
      iv_message    = '' ).
    lv_operational_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-PREVIEW-FILTER'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
      ...
      iv_demand_count     = 1 ).
    lo_cut->finish_run(
      iv_run_id     = lv_operational_run_id
      iv_status     = 'S'
      iv_available  = 4
      iv_allocated  = 4
      iv_shortage   = 0
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = abap_false ).
    DELETE FROM zstockalloc_run
      WHERE run_id = @lv_preview_run_id
         OR run_id = @lv_operational_run_id.
  ENDMETHOD.

  METHOD rejects_invalid_preview_filter.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA lv_raised TYPE abap_bool.
    ...
    TRY.
        lo_cut->get_runs(
          iv_material         = 'MATERIAL-AUDIT-BAD-PREVIEW-FILTER'
          iv_plant            = '1000'
          iv_storage_location = '0001'
          iv_preview_filter   = 'X' ).
      CATCH zcx_stock_allocation INTO DATA(lo_error).
        ...
        cl_abap_unit_assert=>assert_equals(
          act = lo_error->message
          exp = 'Audit preview filter is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
  ENDMETHOD.
  ...
  METHOD latest_success_streak.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA lv_oldest_run_id TYPE zif_allocation_audit=>ty_run_id.
    DATA lv_middle_run_id TYPE zif_allocation_audit=>ty_run_id.
    DATA lv_newest_run_id TYPE zif_allocation_audit=>ty_run_id.
    DATA ls_summary TYPE zif_allocation_audit=>ty_summary.

    CREATE OBJECT lo_cut TYPE zcl_allocation_audit_sap.
    lv_oldest_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-STREAK'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
      iv_available        = '5'
    ...
    lo_cut->finish_run(
      ...
      iv_full_count    = 0
      iv_partial_count = 1
      iv_message       = 'streak regression' ).
    lv_middle_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-STREAK'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
    ...
    lo_cut->finish_run(
      ...
      iv_full_count = 1
      iv_message    = '' ).
    lv_newest_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-STREAK'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
  ...
  METHOD reports_completion_running.
    ...
    lo_cut->finish_run(
      ...
      iv_shortage   = '0'
      iv_full_count = 1
      iv_message    = 'completed diagnostic' ).
    ...
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_run_id
      exp = lv_completed_run_id ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_status
      exp = 'E' ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_message
      exp = 'completed diagnostic' ).
    cl_abap_unit_assert=>assert_not_initial(
      ls_summary-last_completed_start_date ).
    cl_abap_unit_assert=>assert_not_initial(
      ls_summary-last_completed_start_time ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_requested
      exp = 1 ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_allocated
      exp = 1 ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_shortage
      exp = 0 ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_demand
      exp = 1 ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_avail
      exp = 1 ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_avail_unit
      exp = 'EA' ).
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = '201' ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_completed_min_shelf_life
      exp = 5 ).
    cl_abap_unit_assert=>assert_equals(
  ...
  METHOD summarizes_policy_context.
    ...
    cl_abap_unit_assert=>assert_false(
      ls_summary-available_context_ok ).
    cl_abap_unit_assert=>assert_true(
      ls_summary-mixed_available ).
    cl_abap_unit_assert=>assert_true(
      ls_summary-last_avail_ok ).
    lt_runs = lo_cut->get_runs(
      iv_material         = 'MATERIAL-AUDIT-POLICY'
      iv_plant            = '1000'
      iv_storage_location = '0001' ).
    READ TABLE lt_runs WITH KEY run_id = ls_summary-last_run_id
      INTO ls_last_run.
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_avail
      exp = ls_last_run-available ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_avail_unit
      exp = ls_last_run-unit ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_requested
      exp = ls_last_run-requested ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_allocated
      exp = ls_last_run-allocated ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_shortage
      exp = ls_last_run-shortage ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_demand
      exp = ls_last_run-demand_count ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_full
      exp = ls_last_run-full_count ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_partial
      exp = ls_last_run-partial_count ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-last_unalloc
    ...
    cl_abap_unit_assert=>assert_true(
      ls_summary-available_context_ok ).
    cl_abap_unit_assert=>assert_equals(
      act = ls_summary-available_context
      exp = '10' ).
    cl_abap_unit_assert=>assert_false(
      ls_summary-mixed_available ).
    lt_runs = lo_cut->get_runs(
  ...
  METHOD rejects_bad_final_counts.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA lv_run_id TYPE zif_allocation_audit=>ty_run_id.
    DATA lv_raised TYPE abap_bool.
    ...
    CREATE OBJECT lo_cut TYPE zcl_allocation_audit_sap.
    lv_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-FINAL-COUNTS'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
      iv_available        = 1
    ...
    TRY.
        lo_cut->finish_run(
          ...
          iv_full_count = 0
          iv_message    = '' ).
      CATCH zcx_stock_allocation INTO DATA(lo_under_count_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_under_count_error->message
          exp = 'Audit final metrics are invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
  ...
  METHOD rejects_success_message.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA lv_run_id TYPE zif_allocation_audit=>ty_run_id.
    DATA lv_raised TYPE abap_bool.
    DATA lv_status TYPE zif_allocation_audit=>ty_run_status.
    ...
    CREATE OBJECT lo_cut TYPE zcl_allocation_audit_sap.
    lv_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-SUCCESS-MESSAGE'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
      iv_available        = 1
    ...
    TRY.
        lo_cut->finish_run(
          ...
          iv_shortage   = 0
          iv_full_count = 1
          iv_message    = 'unexpected success diagnostic' ).
      CATCH zcx_stock_allocation INTO DATA(lo_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_error->message
          exp = 'Audit final message is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
  ...
  METHOD rejects_negative_policy_read.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA lt_runs TYPE zif_allocation_audit=>tt_runs.
    DATA ls_run TYPE zstockalloc_run.
    DATA lv_raised TYPE abap_bool.
    ...
    ls_run-mandt = sy-mandt.
    ls_run-run_id = 'RUN-AUDIT-NEGATIVE-POLICY'.
    ls_run-matnr = 'MATERIAL-AUDIT-NEGATIVE-POLICY'.
    ls_run-werks = '1000'.
    ls_run-lgort = '0001'.
    ls_run-unit = 'EA'.
    ls_run-start_date = sy-datum.
    ...
    TRY.
        lt_runs = lo_cut->get_runs(
          iv_material         = 'MATERIAL-AUDIT-NEGATIVE-POLICY'
          iv_plant            = '1000'
          iv_storage_location = '0001' ).
      CATCH zcx_stock_allocation INTO DATA(lo_shelf_life_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_shelf_life_error->message
          exp = 'Audit run data is invalid' ).
    ENDTRY.
    DELETE FROM zstockalloc_run
      WHERE run_id = 'RUN-AUDIT-NEGATIVE-POLICY'.
    cl_abap_unit_assert=>assert_true( lv_raised ).

    CLEAR: ls_run, lv_raised.
    ls_run-mandt = sy-mandt.
    ls_run-run_id = 'RUN-AUDIT-NEGATIVE-SAFETY'.
    ls_run-matnr = 'MATERIAL-AUDIT-NEGATIVE-SAFETY'.
    ls_run-werks = '1000'.
    ls_run-lgort = '0001'.
    ls_run-unit = 'EA'.
    ls_run-start_date = sy-datum.
    ...
    ls_run-finish_time = sy-uzeit.
    ls_run-status = 'S'.
    ls_run-safety_stock = -1.
    ls_run-available = 1.
    ls_run-allocated = 1.
    INSERT zstockalloc_run FROM @ls_run.
    TRY.
        lt_runs = lo_cut->get_runs(
          iv_material         = 'MATERIAL-AUDIT-NEGATIVE-SAFETY'
          iv_plant            = '1000'
          iv_storage_location = '0001' ).
      CATCH zcx_stock_allocation INTO DATA(lo_safety_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_safety_error->message
          exp = 'Audit run data is invalid' ).
    ENDTRY.
    DELETE FROM zstockalloc_run
      WHERE run_id = 'RUN-AUDIT-NEGATIVE-SAFETY'.
    cl_abap_unit_assert=>assert_true( lv_raised ).
  ENDMETHOD.

  METHOD rejects_bad_count_read.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA lt_runs TYPE zif_allocation_audit=>tt_runs.
    DATA ls_run TYPE zstockalloc_run.
    DATA lv_raised TYPE abap_bool.
    ...
    ls_run-mandt = sy-mandt.
    ls_run-run_id = 'RUN-AUDIT-INCONSISTENT-COUNTS'.
    ls_run-matnr = 'MATERIAL-AUDIT-INCONSISTENT-COUNTS'.
    ls_run-werks = '1000'.
    ls_run-lgort = '0001'.
    ls_run-unit = 'EA'.
    ls_run-start_date = sy-datum.
    ...
    TRY.
        lt_runs = lo_cut->get_runs(
          iv_material         = 'MATERIAL-AUDIT-INCONSISTENT-COUNTS'
          iv_plant            = '1000'
          iv_storage_location = '0001' ).
      CATCH zcx_stock_allocation INTO DATA(lo_error).
        lv_raised = abap_true.
    ...
    ENDTRY.
    DELETE FROM zstockalloc_run
      WHERE run_id = 'RUN-AUDIT-INCONSISTENT-COUNTS'.
    cl_abap_unit_assert=>assert_true( lv_raised ).
  ENDMETHOD.
  ...
  METHOD rejects_invalid_clock_time.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA ls_run TYPE zstockalloc_run.
    DATA ls_run_view TYPE zif_allocation_audit=>ty_run.
    DATA lv_raised TYPE abap_bool.
    DATA ls_age TYPE zif_allocation_audit=>ty_running_age.

    ls_run-mandt = sy-mandt.
    ls_run-run_id = 'RUN-AUDIT-BAD-CLOCK'.
    ls_run-matnr = 'MATERIAL-AUDIT-BAD-CLOCK'.
    ls_run-werks = '1000'.
    ls_run-lgort = '0001'.
    ls_run-unit = 'EA'.
    ls_run-start_date = '20260102'.
    ...
    TRY.
        lo_cut->get_runs(
          iv_material         = 'MATERIAL-AUDIT-BAD-CLOCK'
          iv_plant            = '1000'
          iv_storage_location = '0001' ).
      CATCH zcx_stock_allocation INTO DATA(lo_error).
        lv_raised = abap_true.
    ...
    cl_abap_unit_assert=>assert_true( lv_raised ).
    DELETE FROM zstockalloc_run
      WHERE run_id = 'RUN-AUDIT-BAD-CLOCK'.

    CLEAR ls_run_view.
    ls_run_view-status = 'R'.
    ls_run_view-start_date = sy-datum.
    ls_run_view-start_time = '120000'.
    ls_age = lo_cut->get_running_age(
      is_run      = ls_run_view
      iv_now_time = '246000' ).
    cl_abap_unit_assert=>assert_false( ls_age-available ).
  ...
  METHOD rejects_malformed_filter_dates.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA lv_raised TYPE abap_bool.
    ...
    TRY.
        lo_cut->get_runs(
          iv_material         = 'MATERIAL-AUDIT-DATE-FILTER'
          iv_plant            = '1000'
          iv_storage_location = '0001'
          iv_start_date_from  = '20260230' ).
      CATCH zcx_stock_allocation INTO DATA(lo_start_date_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_start_date_error->message
          exp = 'Audit date range is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
    CLEAR lv_raised.
    ...
    TRY.
        lo_cut->get_summary(
          ...
          iv_plant             = '1000'
          iv_storage_location  = '0001'
          iv_deadline_age_to   = 1
          iv_deadline_age_date = '20260230' ).
      CATCH zcx_stock_allocation INTO DATA(lo_age_date_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_age_date_error->message
          exp = 'Audit deadline age date is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
    CLEAR lv_raised.
    ...
    TRY.
        lo_cut->get_purge_preview(
          ...
          iv_plant            = '1000'
          iv_storage_location = '0001'
          iv_before_date      = '20260230' ).
      CATCH zcx_stock_allocation INTO DATA(lo_before_date_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_before_date_error->message
          exp = 'Audit purge date is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
    CLEAR lv_raised.
    ...
    TRY.
        lo_cut->purge_runs_before(
          ...
          iv_storage_location = '0001'
          iv_before_date      = '20260230' ).
      CATCH zcx_stock_allocation INTO DATA(lo_purge_date_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_purge_date_error->message
          exp = 'Audit purge date is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
  ENDMETHOD.

  METHOD rejects_bad_purge_time.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA ls_run TYPE zstockalloc_run.
    DATA lv_raised TYPE abap_bool.

    ls_run-mandt = sy-mandt.
    ls_run-run_id = 'RUN-BAD-PURGE-TIME'.
    ls_run-matnr = 'MATERIAL-BAD-PURGE-TIME'.
    ls_run-werks = '1000'.
    ls_run-lgort = '0001'.
    ls_run-unit = 'EA'.
    ls_run-start_date = sy-datum - 2.
    ls_run-start_time = '240000'.
    ls_run-finish_date = sy-datum - 1.
    ls_run-finish_time = '120000'.
    ls_run-status = 'S'.
    ls_run-available = 1.
    ls_run-demand_count = 1.
    ...
    TRY.
        lo_cut->get_purge_preview(
          iv_material         = 'MATERIAL-BAD-PURGE-TIME'
          iv_plant            = '1000'
          iv_storage_location = '0001'
      ...
      CATCH zcx_stock_allocation INTO DATA(lo_preview_error).
        ...
        cl_abap_unit_assert=>assert_equals(
          ...
          exp = 'Audit purge candidate is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
    ...
    cl_abap_unit_assert=>assert_true( lv_raised ).
    DELETE FROM zstockalloc_run
      WHERE run_id = 'RUN-BAD-PURGE-TIME'.

    CLEAR ls_run.
    ls_run-mandt = sy-mandt.
    ls_run-run_id = 'RUN-BAD-PURGE-PAIR'.
    ls_run-matnr = 'MATERIAL-BAD-PURGE-TIME'.
    ls_run-werks = '1000'.
    ls_run-lgort = '0001'.
    ls_run-unit = 'EA'.
    ...
    TRY.
        lo_cut->get_purge_preview(
          ...
          iv_before_date      = sy-datum
          iv_duration_from    = 1 ).
      CATCH zcx_stock_allocation INTO DATA(lo_pair_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_pair_error->message
          exp = 'Audit purge candidate is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
    DELETE FROM zstockalloc_run
      WHERE run_id = 'RUN-BAD-PURGE-PAIR'.
  ENDMETHOD.

  METHOD rejects_bad_purge_horizon.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA ls_run TYPE zstockalloc_run.
    ...
    ls_run-mandt = sy-mandt.
    ls_run-run_id = 'RUN-BAD-PURGE-HORIZON'.
    ls_run-matnr = 'MATERIAL-BAD-PURGE-HORIZON'.
    ls_run-werks = '1000'.
    ls_run-lgort = '0001'.
    ls_run-unit = 'EA'.
    ls_run-start_date = sy-datum - 2.
    ...
    ls_run-finish_date = sy-datum - 1.
    ls_run-finish_time = '120000'.
    ls_run-requested_on_from = '20260230'.
    ls_run-requested_on_to = '20260301'.
    ls_run-status = 'S'.
    ls_run-available = 1.
    ls_run-demand_count = 1.
    INSERT zstockalloc_run FROM @ls_run.
    ...
    cl_abap_unit_assert=>assert_true( lv_raised ).
    DELETE FROM zstockalloc_run
      WHERE run_id = 'RUN-BAD-PURGE-HORIZON'.

    CLEAR ls_run.
    ls_run-mandt = sy-mandt.
    ls_run-run_id = 'RUN-ERROR-PURGE-HORIZON'.
    ls_run-matnr = 'MATERIAL-BAD-PURGE-HORIZON'.
    ls_run-werks = '1000'.
    ls_run-lgort = '0001'.
    ls_run-unit = 'EA'.
    ...
    ls_run-available = 1.
    ls_run-demand_count = 1.
    ls_run-unallocated_count = 1.
    ls_run-shortage = 1.
    ls_run-message = 'diagnostic invalid horizon'.
    INSERT zstockalloc_run FROM @ls_run.
    ...
    lt_runs = lo_cut->get_runs(
      ...
      iv_status            = 'E'
      iv_deadline_age_from = 1 ).
    cl_abap_unit_assert=>assert_initial( lt_runs ).
    lt_runs = lo_cut->get_runs(
      iv_material             = 'MATERIAL-BAD-PURGE-HORIZON'
      iv_plant                = '1000'
      iv_storage_location     = '0001'
    ...
    SELECT COUNT( * )
      FROM zstockalloc_run
      WHERE run_id = 'RUN-ERROR-PURGE-HORIZON' INTO @lv_count.
    cl_abap_unit_assert=>assert_equals(
      act = lv_count
      exp = 1 ).
    DELETE FROM zstockalloc_run
      WHERE run_id = 'RUN-ERROR-PURGE-HORIZON'.
  ENDMETHOD.
  ...
  METHOD protects_reserved_snapshots.
    ...
    ls_allocation-allocated = 1.
    ls_allocation-allocation_status = 'F'.
    ls_allocation-reservation_id = '2000000060'.
    INSERT zstockalloc FROM @ls_allocation.
  ...
  METHOD filters_purge_preview_runs.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    DATA ls_run TYPE zstockalloc_run.
    DATA ls_allocation TYPE zstockalloc.
    DATA ls_preview TYPE zif_allocation_audit=>ty_purge_preview.
    ...
    ls_run-run_id = 'RUN-PURGE-PREVIEW'.
    INSERT zstockalloc_run FROM @ls_run.
    ls_run-preview = abap_false.
    ls_run-run_id = 'RUN-PURGE-OPERATIONAL'.
    INSERT zstockalloc_run FROM @ls_run.

    CREATE OBJECT lo_cut TYPE zcl_allocation_audit_sap.
    ls_preview = lo_cut->get_purge_preview(
      iv_material         = 'MATERIAL-PURGE-PREVIEW'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
      iv_preview_filter   = 'p'
    ...
    SELECT COUNT( * )
      FROM zstockalloc_run
      WHERE run_id = 'RUN-PURGE-OPERATIONAL' INTO @lv_count.
    cl_abap_unit_assert=>assert_equals(
      act = lv_count
      exp = 1 ).
    ...
    SELECT COUNT( * )
      FROM zstockalloc_run
      WHERE matnr = 'MATERIAL-PURGE-PREVIEW' INTO @lv_count.
    cl_abap_unit_assert=>assert_initial( lv_count ).
    DELETE FROM zstockalloc_run
      WHERE matnr = 'MATERIAL-PURGE-PREVIEW'.

    ls_run-matnr = 'MATERIAL-PURGE-MAX'.
    ls_run-preview = abap_false.
    ls_run-start_time = '010002'.
    ls_run-finish_time = '010003'.
    ls_run-run_id = 'RUN-PURGE-MAX-1'.
    INSERT zstockalloc_run FROM @ls_run.
    ls_run-start_time = '010001'.
    ls_run-finish_time = '010002'.
    ls_run-run_id = 'RUN-PURGE-MAX-2'.
    INSERT zstockalloc_run FROM @ls_run.
    ls_run-start_time = '010003'.
    ls_run-finish_time = '010004'.
    ls_run-run_id = 'RUN-PURGE-MAX-3'.
    INSERT zstockalloc_run FROM @ls_run.
    ls_allocation-mandt = sy-mandt.
    ls_allocation-matnr = 'MATERIAL-PURGE-MAX'.
    ls_allocation-werks = '1000'.
    ls_allocation-lgort = '0001'.
    ls_allocation-allocation_unit = 'EA'.
    ls_allocation-order_id = 'PURGE-MAX-003'.
    ls_allocation-run_id = 'RUN-PURGE-MAX-3'.
    ls_allocation-requested = 1.
    ls_allocation-allocated = 1.
    ls_allocation-allocation_status = 'F'.
    ls_allocation-reservation_id = '2000000061'.
    INSERT zstockalloc FROM @ls_allocation.
    ls_preview = lo_cut->get_purge_preview(
      iv_material         = 'MATERIAL-PURGE-MAX'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
      iv_max_runs         = 1
    ...
    SELECT COUNT( * )
      FROM zstockalloc_run
      WHERE matnr = 'MATERIAL-PURGE-MAX' INTO @lv_count.
    cl_abap_unit_assert=>assert_equals(
      act = lv_count
      exp = 2 ).
    SELECT SINGLE run_id
      FROM zstockalloc_run
      WHERE matnr = 'MATERIAL-PURGE-MAX' INTO @lv_remaining_run_id.
    cl_abap_unit_assert=>assert_equals(
      act = lv_remaining_run_id
      exp = 'RUN-PURGE-MAX-1' ).
    lv_deleted = lo_cut->purge_runs_before(
      EXPORTING
        iv_material         = 'MATERIAL-PURGE-MAX'
    ...
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = 1 ).
    DELETE FROM zstockalloc
      WHERE matnr = 'MATERIAL-PURGE-MAX'.
    DELETE FROM zstockalloc_run
      WHERE matnr = 'MATERIAL-PURGE-MAX'.
    ...
    TRY.
        lo_cut->get_purge_preview(
          ...
          iv_max_runs         = -1
          iv_before_date      = sy-datum ).
      CATCH zcx_stock_allocation INTO DATA(lo_max_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_max_error->message
          exp = 'Audit purge maximum run count is invalid' ).
    ENDTRY.
    cl_abap_unit_assert=>assert_true( lv_raised ).
  ENDMETHOD.
  ...
  METHOD rejects_default_commit_returns.
    DATA lo_cut TYPE REF TO zif_allocation_audit.
    ...
    ls_item-material_external = 'MATERIAL-COMMIT-RETURN-ERROR'.
    ...
    TRY.
        lo_cut->record_rejection(
          iv_material         = 'MATERIAL-AUDIT-COMMIT-RETURN'
          iv_plant            = '1000'
          iv_storage_location = '0001'
          iv_unit             = 'EA'
          iv_available        = 0
          iv_message          = 'Default commit return test' ).
      CATCH zcx_stock_allocation INTO DATA(lo_error).
        lv_raised = abap_true.
        cl_abap_unit_assert=>assert_equals(
          act = lo_error->message
    ...
    ENDTRY.
    DELETE FROM zstockalloc_run
      WHERE matnr = 'MATERIAL-AUDIT-COMMIT-RETURN'.
    cl_abap_unit_assert=>assert_true( lv_raised ).

    CLEAR: ls_item, lt_items, lt_return, lv_raised.
    ls_item-material_external = 'MATERIAL-COMMIT-BAD-RETURN'.
    APPEND ls_item TO lt_items.
    lv_run_id = lo_cut->start_run(
      iv_material         = 'MATERIAL-AUDIT-BAD-COMMIT'
      iv_plant            = '1000'
      iv_storage_location = '0001'
      iv_unit             = 'EA'
      iv_available        = 1
    ...
    TRY.
      ...
      CATCH zcx_stock_allocation INTO lo_error.
        ...
        cl_abap_unit_assert=>assert_equals(
          act = lo_error->message
          exp = 'Allocation transaction commit returned invalid status' ).
    ENDTRY.
    DELETE FROM zstockalloc_run
      WHERE matnr = 'MATERIAL-AUDIT-BAD-COMMIT'.
    cl_abap_unit_assert=>assert_true( lv_raised ).
  ENDMETHOD.
