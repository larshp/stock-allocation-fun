...
for (const group of ["mb_bus2093", "mb_bus2017"]) {
  const filename = new URL(`../output/${group}.fugr.mjs`, import.meta.url);
  ...
  if (source.includes(broken)) {
    ...
    console.log(`Applied scoped BAPI RETURN declaration workaround: ${group}`);
