...
- Resolved: stock diagnostics could flag shortages below a floor but could not detect overstock against a ceiling or reject an inverted range. `ZSTOCK_ALLOC_STOCK-p_max` now adds an output-unit-aware upper bound with explicit above-maximum state and min/max validation.
- Resolved: stock conversion output could be consumed only as raw quantity, leaving operators to implement their own minimum-stock decision and risking comparisons in the wrong unit. `ZSTOCK_ALLOC_STOCK-p_min` now evaluates a nonnegative threshold after optional conversion and publishes explicit active/evaluated/below-minimum state.
- Resolved: direct stock diagnostics exposed only the material base-unit quantity, forcing operators to perform a separate conversion call. `ZSTOCK_ALLOC_STOCK` now accepts `p_meins` and emits converted quantity plus base/output-unit provenance through human, CSV, and JSON contracts.
- Resolved: watch used the legacy `p_runq` name for run-ID fragments while other run-oriented reports used `p_rid`, forcing callers to special-case the operational report. Watch now accepts standard `p_rid` alongside the backward-compatible `p_runq`, rejects ambiguous dual input, and publishes the effective filter consistently.
- Resolved: operational health could scope a population by policy and lifecycle windows but not identify a specific audit run or run-ID fragment. `ZSTOCK_ALLOC_HEALTH` now supports `p_runid`/`p_rid` and publishes those filters in human, CSV, JSON, and XML contracts.
...
- Resolved: numeric movement type `000` passed shape checks at several production and persisted-filter boundaries even though it is the SAP zero sentinel and the local BAPI stub rejected it. A shared constant now makes the invalid value explicit and all movement-type boundaries reject it before SAP calls or persistence reads.
- Resolved: sales-order change and reservation-delete test doubles appended BAPI errors but continued through the function, leaving future output or fixture state vulnerable to accidental success. All writer error/status branches now set `sy-subrc` and return immediately.
- Resolved: reservation and goods-movement API test doubles could return an error while still assigning a document identifier, allowing callers that inspected output before return status to look successful. Non-success return branches now stop before document fabrication.
- Resolved: reservation and goods-movement API test doubles treated `NaN` quantities as positive and accepted blank or malformed movement types, allowing invalid writer payloads to look successful. They now require finite positive quantities and three-digit numeric movement types.
- Resolved: the sales-order API test double could accept `NaN` quantities, malformed NUMC keys, or an update row for a different schedule line, allowing schedule mutations to look successful despite a corrupt payload. It now validates SAP key shape, finite positive quantity, and exact row correlation.
- Resolved: reservation and goods-movement API test doubles accepted malformed calendar strings such as impossible month/day combinations, so adapter date-validation regressions could still look successful. The stubs now reject non-SAP or impossible dates before simulating the BAPI operation.
- Resolved: the material-unit conversion API test double could return success for missing units/material or negative/non-finite quantities, allowing invalid direct FM calls to bypass the adapter's input contract. It now raises the standard-call exception before conversion behavior is simulated.
- Resolved: enqueue/dequeue API test doubles accepted missing plant or storage-location scope and could make an incomplete allocation lock look successful. Both stubs now reject incomplete lock keys before simulating SAP success.
- Resolved: the reservation-delete API test double accepted blank, nonnumeric, zero, and wrongly sized reservation identifiers, so malformed cancellation payloads could look successful in local tests. It now rejects those payloads before transaction behavior is simulated.
- Resolved: reservation and goods-movement API test doubles could report an incomplete payload while still fabricating a success document, weakening tests that inspect returned identifiers. They now return the BAPI error before assigning reservation or material-document output.
- Resolved: the sales-order API test double could accept an incomplete schedule-line mutation and still appear successful, allowing adapter payload regressions to evade the local harness. `BAPI_SALESORDER_CHANGE` now returns an error until document/header/update rows and a positive quantity are present.
- Resolved: the goods-movement API test double could emit a material document for an incomplete header, movement code, or item payload, allowing an adapter regression to look successful in the local harness. `BAPI_GOODSMVT_CREATE` now returns an error until the required posting scope and positive quantity are present.
- Resolved: the reservation API test double could return a document for an empty or incomplete header/item payload, so a production adapter regression could be hidden by the local harness. `BAPI_RESERVATION_CREATE1` now rejects missing movement/date, item identity, quantity, unit, or required date fields before returning a document.
- Resolved: SAP table stubs could retain field names while losing datatype, length, date/unit shape, or quantity reference metadata, allowing the local SQL model to diverge from the ABAP values used by production adapters. Repository validation now rejects incomplete or semantically incompatible DDIC field descriptors.
- Resolved: the SAP DDIC field contract was explicit but hand-maintained, so a future qualified field added to a joined standard-table query could be missed until a separate list update. Repository validation now resolves production `alias~field` SQL references through their table aliases and checks each field against the matching SAP stub automatically.
- Resolved: direct callers of the comparison interface could supply negative, over-allocated, internally inconsistent, or status-inconsistent snapshot metrics; production report reads normally passed through a validated sink, but the public comparison port was not fail-closed. Comparison now validates both snapshots before ratio arithmetic and classification.
- Resolved: SAP table-stub contracts checked table identity and client dependence but could miss a field required by production SQL. Repository validation now checks the complete field set used by the stock and order readers against each standard-table stub.
- Resolved: audit commit tests used only an injected exception double, leaving the default SAP transaction adapter's returned-error and unknown-status behavior unverified at the audit boundary. Audit rejection and finalization now run against both real adapter return fixtures and assert the preserved diagnostics.
- Resolved: rollback-return fixtures existed for direct SAP writers but were not all exercised by runtime tests, leaving duplicated reservation, cancellation, goods-issue, and sales-order paths weaker than the explicit transaction adapter regression. Each direct writer now has error and unknown-status rollback regressions with preserved diagnostics.
- Resolved: SAP transaction rollbacks were judged only by `sy-subrc`, so a `BAPI_TRANSACTION_ROLLBACK` return structure with an error or unknown status could be treated as successful compensation. All production rollback boundaries now inspect and validate the returned status, preserve its message, and report the rollback failure.
- Resolved: SAP transaction commits were judged only by `sy-subrc`, so a `BAPI_TRANSACTION_COMMIT` return structure with an error or unknown status could be treated as success. All production commit boundaries now inspect and validate the returned status, preserve its message, and roll back on rejection.
- Resolved: the repository contract enforced custom-table ownership for production `DELETE`, `UPDATE`, and `MODIFY` statements but did not discover Open SQL `INSERT dbtab FROM ...` statements. Production database-write discovery now includes `INSERT`, closing the static guard gap for future standard-table writes.
- Resolved: report and summary consumers trusted `get_runs`' derived requested deadline even when an error run retained an impossible raw horizon. The API now marks that derived deadline unavailable while preserving raw fields for diagnostics.
- Resolved: audit history filtering and deadline-age sorting could interpret malformed requested horizons retained on error runs. Such runs remain available for diagnostics, but deadline-based reads now exclude them before comparisons or subtraction.
- Resolved: audit purge preview and execution could subtract a malformed persisted requested horizon for deadline-age filtering. Non-error candidates now fail validation; error candidates retain diagnostic bounds but are excluded from deadline-based filters before arithmetic.
- Resolved: allocation-result reads could use malformed persisted audit timestamps in duration filters or sort keys before the run reference was validated. Result reads now reject invalid start/finish dates, clock times, timestamp pairing, inversion, and requested horizons before temporal arithmetic.
- Resolved: allocation-result filters checked date ordering but not calendar validity, so impossible dates could reach deadline, reservation, or requested-date comparisons and age arithmetic. The sink now validates every caller-supplied date boundary before use.
- Resolved: comparison running-age calculation checked only status and blankness before timestamp subtraction, so malformed times or half-populated finish timestamps could reach arithmetic. The comparison API now validates calendar/clock values and timestamp pairing, returning unavailable for corrupt running rows.
- Resolved: audit purge preview and execution checked candidate timestamp fields only for blankness, so malformed persisted times could reach duration subtraction. Both purge paths now validate start/finish dates, clock values, timestamp pairing, and temporal order before arithmetic.
- Resolved: injected stock providers could return batch-only metadata for an aggregate no-batch read, and the service validated the flags without validating that cross-field relationship. No-batch results now reject batch existence, restriction, or expiration metadata before allocation.
- Resolved: date validation initially covered SAP stock/order rows but not all caller and persistence boundaries. Allocation, audit, and order APIs now reject malformed requested-date bounds; snapshot demand/reservation dates and persisted run dates are validated before use, with the intentional exception that error runs preserve invalid input bounds for diagnostics.
- Resolved: the direct SAP reservation adapter rejected an initial required date but could pass an impossible non-initial date to `BAPI_RESERVATION_CREATE1`. Required dates are now validated before authority checks and the BAPI call.
- Resolved: injected allocation snapshots could contain impossible demand or reservation dates even though persisted snapshots were validated by the SAP sink. The service now rejects malformed existing snapshot dates before reconciliation side effects.
- Resolved: audit history and retention filters checked date ordering but not calendar validity, so impossible dates could reach comparisons or deadline-age arithmetic. All audit filter boundaries now validate calendar dates before use.
- Resolved: audit run and running-age boundaries checked timestamps only for blankness, so impossible `TIMS` values could reach duration arithmetic. A shared clock validator now rejects invalid hours, minutes, and seconds.
- Resolved: SAP date fields were compared as strings without calendar validation, so malformed non-initial batch-expiration or requested-delivery values could affect shelf-life, delivery-date compatibility, or horizon selection. A shared date validator now rejects impossible dates at the source and service boundaries, and the order reader validates candidates before applying the requested-date range.
- Resolved: production BAPI and transaction calls relied on return tables or `sy-subrc` without consistently mapping classic function-module exceptions, allowing a real SAP exception to escape the adapter boundary. All production function-module calls now catch `OTHERS = 1`, preserve rollback diagnostics, and are covered by thrown-exception stubs and ABAP Unit regressions.
- Resolved: stock-read authorization always required `MARD`, even when a batch read queried `MCHB` and `MCHA` instead. The authority adapter now checks only the storage-stock table used by the selected path, while retaining common material/plant checks.
- Resolved: the SAP order reader filtered `VBEP-WMENG <= VBEP-BMENG` before validating quantities, so a negative requested quantity could disappear when confirmation was nonnegative. The SQL now retains open or negative-quantity rows, validates corrupt quantities first, and skips only genuinely fulfilled nonnegative schedules.
- Resolved: the repository verified the `open-abap-core` package and tool configuration independently, so a future lockfile update could desynchronize the installed archive from the declared dependency. Repository validation now requires the package manifest, lockfile root, and resolved package entry to match exactly.
- Resolved: classic SAP function-module exceptions in material conversion and allocation locking were not explicitly mapped, so a real SAP exception could escape the application error contract. These calls now use `EXCEPTIONS OTHERS = 1` and preserve their existing typed `sy-subrc` diagnostics.
- Resolved: source-placement enforcement covered Z/Y ABAP source files but could leave matching abapGit metadata outside `src/`, creating an incomplete custom-object boundary. Repository validation now rejects Z/Y class, interface, program, and table XML outside the custom source directory.
- Resolved: direct-write report safety was only partially protected by repository contracts, so a future report could bypass explicit execution or adapter authority wiring without a focused failure. Reservation create/cancel, goods issue, and sales-order update now have shared contract checks for `p_exec`, authority injection, and adapter commit/rollback boundaries.
- Resolved: CSV errors exposed only `mode`, `status`, and `message`, so consumers could not select the matching report contract; all production CSV error envelopes now carry the applicable schema version and allocation run ID where available.
- Resolved: custom table metadata was not protected by the same client-isolation invariant as SAP table stubs; every custom transparent table now requires `CLIDEP=X` and keyed `MANDT` in repository validation.
- Resolved: source placement was checked only for currently enumerated `src` objects, so a future Z-namespaced ABAP file could be added elsewhere unnoticed; repository validation now rejects such objects outside `src/`.
- Resolved: comparison documentation described historical schema `36` as active despite the executable contract having advanced to `95`; the wording now distinguishes history from the current contract and is regression-tested.
- Resolved: verification was only enforced by local commands, so regressions could bypass the documented lint/transpile/unit/contract gates; the repository now runs the full pipeline in CI for pushes and pull requests.
- Resolved: result success JSON used detail/summary schemas `37`/`43`, but its validation, authorization, and read errors were unversioned; result now selects the matching schema from summary mode for every JSON error path.
- Resolved: comparison contextual JSON used schema `95` on successful output while validation, authorization, and read failures returned unversioned errors; all 102 comparison JSON error paths now carry schema `95`.
- Resolved: report-level schema versioning could regress silently because the repository contract counted current calls but did not reject future legacy JSON error calls; every production report that emits JSON now requires `error_with_schema` and rejects the legacy helper.
- Resolved: SAP table descriptors could drift toward client-unsafe or custom-table fixtures without a repository-level guard; all standard stubs now require a keyed `MANDT` field and non-`Z` table identity.
- Resolved: ownership of production SQL writes was protected only by the lint rule and could regress without a repository contract assertion; production database-style writes are now discovered and required to target `Z*` tables.
- Resolved: history success JSON used detail/summary schemas `26`/`43`, but its validation, authorization, and read errors were unversioned; history now selects the matching schema from summary mode for every JSON error path.
- Resolved: watch JSON/NDJSON success contexts used schema `59`, but mode, validation, and audit-read failures returned unversioned errors; watch JSON errors now carry schema `59`.
- Resolved: the health report exposed schema `28` only on successful JSON output while validation and audit-read failures remained unversioned; all health JSON errors now carry schema `28`.
- Resolved: purge JSON success responses exposed preview/execution schemas `22`/`23`, but validation and retention failures were unversioned. Purge now uses the matching schema for every JSON error path, selected from `p_exec` before validation.
- Resolved: `ZSTOCK_ALLOCATE` success JSON used schema `34` while validation, authorization, allocation, and summary failures returned unversioned errors; allocation JSON errors now use schema `34` and retain the correlated `run_id` when available.
- Resolved: direct SAP operator reports versioned successful JSON responses only; validation and adapter failures still emitted an unversioned `{mode,message}` envelope. The shared JSON helper and all six direct reports now emit `schema_version: 1` on JSON errors as well.
- Resolved: the stock reader could accept a material and stock row even when the selected material/plant relation had no `MARC` row. Existing material master data is now required to include plant data before stock can be allocated; genuinely missing MARA materials still retain the not-found result.
- Resolved: after the stock reader began checking plant-specific `MARC-LVORM`, its default `S_TABU_NAM` adapter did not yet authorize the new MARC read. Stock-read authorization now checks activity `03` for MARC before the query.
- Resolved: the stock reader checked client-level, storage-location, batch-stock, and batch-master deletion flags but omitted `MARC-LVORM`, allowing a material/plant relation flagged for deletion to remain allocatable. The `MARC` stub and SAP stock source now validate and reject plant-specific deletion markers, including malformed flag values.
- Resolved: the order reader filtered header and schedule-line delivery blocks but did not read `VBAP-LIFSP`, so an item-level delivery block could still become allocation demand. The `VBAP` stub and SAP order source now exclude item-blocked lines before mapping open demand.
- Resolved: purge preview and execution exposed schema versions only for typed JSON, leaving default JSON consumers without an explicit contract version. `ZSTOCK_ALLOC_PURGE` now emits schemas `22` and `23` in both JSON shapes.
- Resolved: the primary allocation report exposed its machine-readable schema marker only for typed JSON, leaving default JSON consumers without an explicit contract version. `ZSTOCK_ALLOCATE` now emits schema `34` in both JSON success shapes.
- Resolved: direct SAP operator reports exposed schema versioning only when typed JSON was selected, leaving untyped JSON consumers without a machine-readable contract marker. The stock, conversion, reservation create/cancel, goods-issue, and sales-order-update reports now emit `schema_version: 1` in both JSON success shapes.
- Resolved: the README described SAP sales-order identities as numeric, but the order source and injected snapshot/result paths accepted exact-length alphanumeric values. The shared ten-character numeric contract is now enforced at source, service, snapshot, and result-filter boundaries, with valid fixtures migrated to ten-digit keys and malformed-key regressions retained.
- Resolved: material-unit conversion was only observable indirectly through allocation and reservation flows, making adapter diagnosis harder. `ZSTOCK_ALLOC_CONVERT` now exposes the existing material-specific conversion contract as a read-only report while preserving canonical unit provenance and typed numeric output.
- Resolved: direct reservation creation had no controlled operator entry point outside the allocation service, while reservation cancellation was already exposed for cleanup. `ZSTOCK_ALLOC_RESERVE` now requires explicit execution and delegates unit normalization, validation, authority, BAPI return inspection, commit, and rollback handling to the existing adapter.
- Resolved: reservation cleanup was available through allocation orchestration but lacked a controlled operator entry point for an already-known SAP reservation. `ZSTOCK_ALLOC_RES_CANCEL` now requires an explicit execution checkbox and delegates validation, authorization, BAPI return inspection, rollback, and commit handling to the existing reservation adapter.
- Resolved: stock availability could be read only indirectly through allocation or health flows, making direct operator diagnosis unnecessarily dependent on audit data. `ZSTOCK_ALLOC_STOCK` now exposes the existing SAP stock-source contract as a read-only report without adding a second database-read implementation.
- Resolved: the allocation, goods-issue, and sales-order update reports delegated lowercase unit/order-type normalization to their adapters but echoed the original casing in preview and export output. All three reports now canonicalize once at the report boundary and use the canonical value for the SAP call and every output shape.
- Resolved: history, result, watch, and purge reports passed lowercase unit filters into canonical readers but still exposed the raw casing in filter provenance. Those reports now normalize the selection parameter before filtering and rendering output.
- Resolved: result filters for lifecycle status, sales-document type, order unit, and reservation unit, plus the history lifecycle status filter, could preserve lowercase input in downstream calls and provenance. Both reports now canonicalize those case-insensitive SAP codes at the report boundary.
- Resolved: comparison old/new sales-document-type filters were the remaining case-sensitive SAP code inputs in the report boundary. Common, old, and new type filters are now uppercased before reads, classification, and exports.
- Resolved: comparison change-type validation rejected lowercase `p_chg` even though the report's other code filters were case-insensitive. The report now uppercases the value before validation and output.
...
- Resolved: comparison could filter reservation movement but not the associated persisted reservation unit; `p_runit` plus `p_orunit`/`p_nrunit` now restrict old/new snapshot populations with explicit conflict validation and schema `69` provenance.
- Resolved: comparison exposed reservation movement types only as row data and could not restrict old/new snapshot populations independently; `p_rmov` plus `p_ormov`/`p_nrmov` now apply effective reservation-movement filters with validation and schema `68` provenance.
- Resolved: comparison required one shared allocation unit even when old and new audit runs used different units; `p_ounit`/`p_nunit` now select old/new reads independently, retain common `p_meins` compatibility, reject ambiguous common-plus-side filters, and expose both unit filters in schema `67` provenance.
- Resolved: retention could filter by lifecycle and delivery policy but could not target persisted diagnostic-bearing runs; purge preview and execution now support case-insensitive `p_msg` and nonblank `p_monly` selection with explicit provenance.
- Resolved: comparison could report old/new audit diagnostics but could not select each originating run by message content or message presence; `p_omsg`/`p_nmsg` and `p_omonly`/`p_nmonly` now constrain old/new run and snapshot resolution independently with explicit provenance.
- Resolved: result consumers could not isolate snapshot rows from runs by persisted diagnostic message; `p_msg`/`p_monly` now constrain latest/exact audit resolution and sink reads with explicit provenance.
- Resolved: result consumers could prioritize audit deadline urgency or snapshot requested dates, but not the earliest originating requested deadline; `p_due` now orders effective originating deadlines first and keeps no-deadline runs last with deterministic ties.
- Resolved: comparison could constrain snapshot-line outcomes but not the lifecycle state of either originating audit run; `p_oast`/`p_nast` now apply independent validated status filters to old/new run and snapshot resolution with explicit provenance.
- Resolved: result consumers could filter snapshot-line status but not the persisted lifecycle status of the originating audit run; `p_astat` now applies validated `R`/`S`/`P`/`E` filtering to result reads and latest-run selection with explicit export provenance.
- Resolved: result rows exposed originating audit context but could not prioritize rows from the slowest completed audit runs; `p_tdur` now sorts by persisted audit duration, keeps unavailable durations last, and result schemas record the new sort contract.
- Resolved: result rows exposed originating audit runs but could not prioritize rows from runs affecting the most demand lines; `p_dcnt` now uses persisted run demand counts with shortage and deterministic date/unit/priority/run/order tie breakers, and result schemas record the new sort contract.
- Resolved: history could filter runs by demand count but could not prioritize the runs affecting the most demand lines; `p_dcnt` now sorts descending demand count with shortage and deterministic start/run-ID tie breakers, and detail/summary export schemas record the new sort contract.
- Resolved: stale-run watch could filter by demand count but could not prioritize the runs affecting the most demand lines; `p_dcnt` now sorts descending demand count with shortage and deterministic age/start/run-ID tie breakers, and export schemas record the new sort contract.
- Resolved: comparison triage could filter outcome states but could not rank worsening state transitions; `p_sreg` now orders full-to-unallocated and other regressions before stable or improved rows with safe non-comparable-row handling.
- Resolved: comparison consumers could not isolate full, partial, or unallocated outcomes on either snapshot side; `p_ost`/`p_nst` now apply validated old/new allocation-status filters with explicit missing-side semantics and export provenance.
...
- Resolved: operational health could target requested horizons but not isolate overdue effective deadlines; `ZSTOCK_ALLOC_HEALTH` now accepts `p_ovrd`/`p_odate`, applies the canonical effective-deadline predicate, and exports the selected overdue provenance.
- Resolved: operational health could isolate overdue deadlines but could not explicitly require a deadline-bearing population; `ZSTOCK_ALLOC_HEALTH` now accepts `p_dead` and exports that selection in every success output mode.
- Resolved: operational health could require a requested deadline but could not constrain its effective date; `ZSTOCK_ALLOC_HEALTH` now accepts inclusive `p_deadf`/`p_deadt` bounds and rejects reversed ranges before any SAP read.
- Resolved: operational health could select deadline ranges but could not express urgency relative to an as-of date; `ZSTOCK_ALLOC_HEALTH` now accepts signed deadline-age bounds with explicit reference-date provenance and canonical audit filtering.
- Resolved: health consumers could see deadline filter inputs but not the selected population’s effective deadline telemetry; health output now carries deadline counts, range, signed age bounds, and exact reference-date provenance from the audit summary.
- Resolved: health consumers could not isolate one audit lifecycle outcome; `ZSTOCK_ALLOC_HEALTH` now accepts `p_stat` and exports the normalized audit-status scope.
- Resolved: health consumers could not isolate runs with diagnostic audit messages; `ZSTOCK_ALLOC_HEALTH` now supports case-insensitive message substring and nonblank-message filtering with explicit provenance.
- Resolved: health consumers could not limit the operational population by originating demand volume; `ZSTOCK_ALLOC_HEALTH` now supports inclusive demand-count bounds with canonical audit validation.
- Resolved: health consumers could not limit the population by persisted available-stock baseline; `ZSTOCK_ALLOC_HEALTH` now supports inclusive available-stock bounds with canonical audit validation.
- Resolved: health consumers could not limit the population by persisted requested quantity; `ZSTOCK_ALLOC_HEALTH` now supports inclusive requested-quantity bounds with canonical audit validation.
- Resolved: health consumers could not limit the population by persisted allocated quantity; `ZSTOCK_ALLOC_HEALTH` now supports inclusive allocated-quantity bounds with canonical audit validation.
- Resolved: health consumers could not limit the population by persisted shortage quantity; `ZSTOCK_ALLOC_HEALTH` now supports inclusive shortage-quantity bounds with canonical audit validation.
...
- Resolved: retention could target one exact audit run but could not select a recognizable run-ID family; purge preview and execution now support case-insensitive `p_rid` substring scoping with explicit exact/substring provenance.
- Resolved: retention could only express an upper cutoff and could not target a bounded historical window; purge preview and execution now accept `p_from` as an inclusive audit-start lower bound with explicit validation and provenance.
- Resolved: purge `p_from` initially required a broad candidate read because the optional host-variable `OR` predicate was not transpiler-compatible; separate blank/bounded `SELECT` branches now push the inclusive lower bound into SQL while keeping the defensive predicate for parity protection.
- Resolved: retention could constrain audit starts but could not target a completed-run finish window; purge now applies inclusive `p_ffrom`/`p_fto` bounds, excludes unfinished rows while active, and preserves the same rule in preview and execution.
- Resolved: retention could filter lifecycle status and policy values but not the persisted allocator strategy; purge now accepts the same canonical strategy set as history and result reads and applies it consistently before deletion.
- Resolved: retention could target persisted strategies but could not isolate blank-strategy pre-persistence history; purge now supports an explicit legacy-strategy filter with the same conflict safeguards used by history, result, and watch.
- Resolved: comparison could scope old/new runs by lifecycle status and diagnostics but not persisted allocator strategy; independent old/new strategy filters now keep audit and snapshot populations aligned.
- Resolved: comparison could scope old/new runs by persisted strategy but not isolate blank-strategy legacy history; independent old/new legacy switches now keep audit and snapshot populations aligned with conflict safeguards and explicit provenance.
- Resolved: comparison used one shared requested-delivery horizon for both runs, making asymmetric window migrations awkward; independent old/new horizon bounds now select audit populations separately while keeping snapshot reads aligned to the selected run.
- Resolved: comparison used one shared deadline-bearing predicate for both runs, making deadline adoption comparisons awkward; independent old/new deadline-only switches now select audit and snapshot populations separately with explicit conflict safeguards.
- Resolved: comparison used one shared effective-deadline range for both runs, making delivery-window changes awkward to compare; independent old/new deadline ranges now select audit and snapshot populations separately with reversed-range and conflict safeguards.
- Resolved: comparison used one shared signed deadline-age range for both runs, making deadline-aging changes awkward to compare; independent old/new age ranges now select audit and snapshot populations separately while preserving one explicit reference date.
- Resolved: comparison used one shared overdue predicate for both runs, making overdue-policy migrations awkward to compare; independent old/new overdue switches now select audit and snapshot populations separately while preserving the shared as-of date and guard safety.
- Resolved: comparison used one shared originating movement-type policy for both runs, making policy migrations awkward to compare; independent old/new movement filters now select audit populations separately with explicit conflict validation.
- Resolved: comparison used one shared minimum-shelf-life policy for both runs, making batch-policy migrations awkward to compare; independent old/new shelf-life filters now select audit populations separately with nonnegative and conflict validation.
- Resolved: mixed-unit audit summaries suppressed most strategy totals but left weighted requested, allocated, and shortage quantities populated; weighted telemetry is now cleared with the other non-comparable quantity aggregates.
- Resolved: an ABAP Unit test passed a signed text literal to a NUMC-typed sales-item parameter, preventing lint from compiling the test; the regression now uses the representable zero-key input at the interface boundary.
- Resolved: health evaluation treated an explicitly queried zero stale-run count as an omitted value, allowing an old run in the broader scope to trigger a false critical alert; the report now marks the stale scope as evaluated and the evaluator honors zero.
- Resolved: operational health could filter by persisted strategies but not isolate blank-strategy legacy runs; `ZSTOCK_ALLOC_HEALTH` now supports an explicit, conflict-checked legacy-strategy filter with exported provenance.
- Resolved: operational health could filter shortage quantity but not the persisted shortage percentage; `ZSTOCK_ALLOC_HEALTH` now exposes independent 0-through-100 `p_spf`/`p_spt` bounds alongside the separate `p_spct` warning threshold.
- Resolved: operational health could filter shortage percentage but not persisted coverage percentage; `ZSTOCK_ALLOC_HEALTH` now exposes independent 0-through-100 `p_covf`/`p_covt` bounds alongside the separate `p_cov` warning threshold.
- Resolved: operational health could scope quantities and percentages but not completed audit duration; `ZSTOCK_ALLOC_HEALTH` now exposes nonnegative `p_tfrom`/`p_tto` bounds and excludes running audits when duration filtering is active.
