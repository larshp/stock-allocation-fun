...
CLASS zcl_alloc_changes IMPLEMENTATION.
  ...
  METHOD run.
    ...
    LOOP AT mo_store->latest_per_material( iv_werks ) INTO DATA(ls_now).
      ...
      DATA(lv_had) = confirmed_before(
        it_before    = lv_before
        iv_demand_id = ls_now-demand_id ).

      " a line that is exactly where it was is not news
      IF lv_had = ls_now-confirmed.
        CONTINUE.
      ENDIF.

      IF iv_worse_only = abap_true AND ls_now-confirmed >= lv_had.
        CONTINUE.
      ENDIF.

      IF ls_now-confirmed < lv_had.
        lv_lost = lv_lost + 1.
      ENDIF.
      lv_shown = lv_shown + 1.
      ...
      APPEND format_row(
        ...
        iv_before = |{ lv_had }|
        iv_now    = |{ ls_now-confirmed }| ) TO rt_line.
