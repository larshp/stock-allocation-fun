...
`P_STRAT` selects how constrained stock is distributed among demands with the same priority. `F` is strict FIFO by material-availability date and remains the default. `P` allocates proportionally to requested quantity. `E` applies max-min fair sharing: small demands are completed first, then the remaining stock is divided equally among larger demands. `S` serves the smallest demands first, maximizing the number of fully supplied schedule lines in the tier. All strategies exhaust higher-priority tiers before considering lower-priority tiers, use sales-order keys as deterministic tie breakers, conserve stock through three-decimal rounding, and never allocate more than requested. The effective code is shown in report output, persisted per allocation row, and recorded in the application-log summary.

Select `P_COMP` to compare all four policies in one side-effect-free run. The service reads and rechecks stock once, reads demand once, and calculates every candidate from that same snapshot. The report displays full, partial, and unallocated demand counts, quantity fill percentage, complete-demand service-level percentage, and allocated and shortage quantities for each strategy; comparison never locks, logs, persists, or commits.
...
5. Compare `F`, `P`, `E`, and `S` simulations for one constrained priority tier, test both blank and dated `P_CUTOF`, then reconcile each calculation with `MARD-LABST`, `MARA-MEINS`, and the selected positive `VBBE-OMENG` schedule lines.
