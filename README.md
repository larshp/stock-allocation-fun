...
Allocation statuses are `F` (full), `P` (partial), and `N` (none). Quantities carry the material base unit read from `MARA`, the unit in which SAP manages material stock. Results retain the sales-order schedule-line key, so multiple requirements for one item cannot overwrite each other. Before any plan can be returned, logged, or persisted, an invariant gate verifies quantity arithmetic, statuses, row context, and total stock conservation. Re-running the report atomically replaces the `ZSTOCKPLAN` scope header and its `ZSTOCKALLOC` detail rows. The header preserves stock context, strategy, window, cardinality, and audit provenance even when a successful plan contains no demand rows.
...
The allocation report starts in simulation mode. Leave `P_SIM` selected to calculate and display a fresh plan—including the selected reserve buffer—without enqueueing, logging, persisting, or committing. Clear it only when the displayed scope should replace the saved allocation plan. Report `ZSTOCK_PLAN_VIEW` reads the last committed header and details without recalculation, requires display activity `03`, reports a persisted empty plan distinctly from a never-planned scope, and rejects inconsistent stored header/detail data. Its non-negative `P_MAXAGE` threshold defaults to one day; older valid plans remain visible but carry an explicit stale warning and age in days.
