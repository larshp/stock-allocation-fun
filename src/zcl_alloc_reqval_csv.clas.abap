CLASS zcl_alloc_reqval_csv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS build
      IMPORTING
        it_issues       TYPE zcl_alloc_request_validator=>ty_issue_tt
      RETURNING
...
CLASS zcl_alloc_reqval_csv IMPLEMENTATION.
  ...
  METHOD build.
    ...
    APPEND 'FIELD_NAME' TO lt_fields.
    ...
    LOOP AT it_issues INTO DATA(ls_issue).
      CLEAR lt_fields.
      APPEND |{ ls_issue-index }| TO lt_fields.
      APPEND |{ ls_issue-field_name }| TO lt_fields.
