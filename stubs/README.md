...
failing stub. The pinned transpiler emits the BAPI RETURN parameter without a workaround.
...
ABAP subclasses; both issue adapters exercise the real function call against the error stub.
The item subset includes RESERV_NO, RES_ITEM and RES_TYPE for reservation-referenced
goods issues, plus WITHDRAWN to verify that adapters never force final issue.
