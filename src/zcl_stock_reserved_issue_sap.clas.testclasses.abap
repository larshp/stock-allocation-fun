CLASS lcl_gateway DEFINITION FINAL INHERITING FROM zcl_stock_reserved_issue_sap.
  PUBLIC SECTION.
...
CLASS ltcl_reserved_issue DEFINITION FINAL FOR TESTING DURATION SHORT RISK LEVEL HARMLESS.
  PRIVATE SECTION.
    ...
    METHODS setup.
    METHODS maps_reserved_issue FOR TESTING RAISING zcx_stock_alloc.
    ...
    METHODS requires_reservation_reference FOR TESTING.
    METHODS rejects_duplicate_references FOR TESTING.
    METHODS distinguishes_reference_keys FOR TESTING RAISING zcx_stock_alloc.
    METHODS consumes_order_simulation FOR TESTING RAISING zcx_stock_alloc.
    METHODS accepts_manual_reservation FOR TESTING RAISING zcx_stock_alloc.
...
CLASS ltcl_reserved_issue IMPLEMENTATION.
  METHOD setup.
    gateway = NEW #( ).
    writer = gateway.
    ...
    allocations[ 1 ]-origin = VALUE #( order_id = '000000001000' reservation = '0000000100'
                                       reservation_item = '0001' reservation_type = '1' ).
    allocations[ 2 ]-origin = VALUE #( reservation = '0000000100' reservation_item = '0002' ).
  ENDMETHOD.

  METHOD maps_reserved_issue.
    ...
    cl_abap_unit_assert=>assert_equals( act = lines( gateway->captured_items )
                                      exp   = 1 ).
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 1 ]-reserv_no
                                      exp   = '0000000100' ).
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 1 ]-res_item
                                      exp   = '0001' ).
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 1 ]-res_type
                                      exp   = '1' ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 1 ]-material ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 1 ]-plant ).
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 1 ]-stge_loc
                                      exp   = '0001' ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 1 ]-move_type ).
    ...
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 1 ]-entry_uom
                                      exp   = 'EA' ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 1 ]-costcenter ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 1 ]-mvt_ind ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 1 ]-withdrawn ).
    cl_abap_unit_assert=>assert_false( gateway->captured_test ).
  ENDMETHOD.
  ...
  METHOD requires_reservation_reference.
    DATA(original) = allocations.
    CLEAR allocations[ 1 ]-origin.
    assert_rejected_before_call( ).
    allocations[ 1 ]-origin = VALUE #( order_id = '000000001000' ).
    assert_rejected_before_call( ).
    allocations[ 1 ]-origin = VALUE #( reservation = '0000000100' ).
    assert_rejected_before_call( ).
    allocations[ 1 ]-origin = VALUE #( reservation_item = '0001' ).
    assert_rejected_before_call( ).
  ...
  METHOD rejects_duplicate_references.
    allocations[ 2 ]-origin = allocations[ 1 ]-origin.
    allocations[ 2 ]-origin-order_id = '000000002000'.
    assert_rejected_before_call( ).
    allocations[ 2 ]-allocated = 2.
    allocations[ 2 ]-shortage = 0.
    assert_rejected_before_call( ).
  ENDMETHOD.

  METHOD distinguishes_reference_keys.
    allocations[ 2 ]-origin = allocations[ 1 ]-origin.
    CLEAR allocations[ 2 ]-origin-reservation_type.
    allocations[ 2 ]-allocated = 2.
    allocations[ 2 ]-shortage = 0.
    writer->create( allocations   = allocations
                    posting_date  = '20260906'
    ...
    cl_abap_unit_assert=>assert_equals( act = lines( gateway->captured_items )
                                      exp   = 2 ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 2 ]-res_type ).
    allocations[ 2 ]-origin-reservation_type = '1'.
    allocations[ 2 ]-origin-reservation = '0000000200'.
    writer->create( allocations   = allocations
                    posting_date  = '20260906'
                    document_date = '20260905' ).
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 2 ]-reserv_no
                                      exp   = '0000000200' ).
  ENDMETHOD.
  ...
  METHOD consumes_order_simulation.
    ...
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 1 ]-entry_qnt
                                      exp   = 8 ).
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 2 ]-entry_qnt
                                      exp   = 2 ).
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 2 ]-res_item
                                      exp   = '0002' ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 2 ]-withdrawn ).
  ENDMETHOD.

  METHOD accepts_manual_reservation.
    CLEAR: allocations[ 1 ]-origin-order_id, allocations[ 1 ]-origin-reservation_type.
    writer->create( allocations   = allocations
                    posting_date  = '20260906'
                    document_date = '20260905' ).
    cl_abap_unit_assert=>assert_equals( act = gateway->captured_items[ 1 ]-reserv_no
                                      exp   = '0000000100' ).
    cl_abap_unit_assert=>assert_initial( gateway->captured_items[ 1 ]-res_type ).
  ENDMETHOD.
ENDCLASS.
