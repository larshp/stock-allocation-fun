CLASS ltcl_alloc_export_alloc_fw DEFINITION
  FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS
  FINAL.
  ...
  PRIVATE SECTION.
    DATA mo_cut TYPE REF TO zcl_alloc_export_alloc_fw.
    ...
    METHODS width_is_fixed FOR TESTING.
    METHODS row_content  FOR TESTING.
ENDCLASS.


CLASS ltcl_alloc_export_alloc_fw IMPLEMENTATION.

  METHOD setup.
    mo_cut = NEW zcl_alloc_export_alloc_fw( ).
  ENDMETHOD.
  ...
  METHOD header_only.
    ...
    cl_abap_unit_assert=>assert_equals( act = strlen( lt_lines[ 1 ] )
                                        exp = 66 ).
  ENDMETHOD.

  METHOD width_is_fixed.
    DATA lt_result TYPE zcl_stock_allocator=>ty_result_tt.
    ...
    cl_abap_unit_assert=>assert_equals( act = strlen( lt_lines[ 2 ] )
                                        exp = 66 ).
  ENDMETHOD.
  ...
  METHOD row_content.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = find( val = lt_lines[ 2 ] sub = 'MAT-1' )
      exp = 20 ).
  ENDMETHOD.
