...
**Batch 5 (verification and hardening) is in progress: orders 444-446 are delivered
and verified; the 443 orders of batches 1-4 remain complete.**
...
files no longer apply; batch 4 is complete and batch 5 above is the active work.

Next up (in order):

1. 447 `zcl_alloc_regression_baseline` - capture a run signature and diff two runs.
2. 448 `zcl_alloc_scenario_matrix` - one base result under several parameter sets.
3. 449 `zcl_alloc_e2e_scenario` - load a scenario into the stub tables and run the
   full service flow (the end-to-end smoke test).

Then batch 5 is complete and a further batch needs to be planned first.
...
* `npm run clean` uses `rimraf output` (`rimraf` is a dev dependency) instead of a
  `node -e` one-liner.
* Metadata exists for every class (roadmap orders 1-446). The classes from order
