...
- Added a repository guard for selected-batch reservation scope: RESB netting includes the selected batch plus unassigned reservations, while excluding reservations assigned to other batches.
- Added explicit batch-assigned and unassigned open RESB totals to the stock-source result and `ZSTOCK_ALLOC_STOCK` human, CSV, JSON, typed JSON, and metadata-summary output. ABAP Unit verifies the split for aggregate, selected-batch, and unassigned-only commitments; schemas advance to stock JSON/CSV `30` and metadata JSON `31`.
- Added exact `p_runid` and case-insensitive substring `p_rid` filters to `ZSTOCK_ALLOC_STOCK-p_net`; both are passed into snapshot retrieval, can be combined, and appear in human, CSV, JSON, metadata filters, and typed filter values.
