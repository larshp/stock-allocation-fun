...
| -------------------- | -------------------------------------------------------- |
| `ZSTOCK_ALLOC_PLTS`  | how every plant you may see stands, and can be mailed when one of them is short, did not run or has a transfer nobody has answered: materials, short lines, how much is short, the oldest day anything is still waiting for, whether a run has touched it today, and how many proposed transfers are waiting |
| `ZSTOCK_ALLOC_SHORT` | what is short across the plant, soonest and biggest first, with the unit, how long it has been short, the reason, the customer and how much somebody has already proposed a transfer for on every line, narrowed to one MRP controller or one customer, and orderable by what has waited longest instead of what is wanted soonest: the list to work through |
...
| `ZSTOCK_ALLOC_ELSE`  | which other plants you may see are sitting on what this one is short of, what is waiting for it there, and what is therefore spare, and who to ring about it there — each plant's stock and demand read the way that plant reads its own, with the ones somebody has already made a note about marked and what another plant has already been promised taken off |
