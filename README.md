...
5. Compare `F`, `P`, `E`, `S`, and `C` simulations for one constrained priority tier, verify `P_OBJ` recommendations for service, quantity, fairness, and due-date urgency goals, test both blank and dated `P_CUTOF`, then reconcile each calculation with `MARD-LABST`, `MARA-MEINS`, and the selected positive `VBBE-OMENG` schedule lines.
