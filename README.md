...
The `ZSTOCK_ALLOC_PURGE` report executes audit retention only when `p_exec` is selected and rejects future cutoff dates. Material, plant, storage location, optional batch/unit, and the cutoff date scope the deletion; running audit rows remain protected by the audit service.
