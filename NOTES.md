...
**Batch 5 (verification and hardening) is complete: orders 444-449 are delivered
...
files no longer apply; batches 4 and 5 are both complete and there is no next
feature. Adding more needs a new batch agreed in `PLAN.md` first, so that scope is
a decision rather than an improvisation; the loop stays the same (feature, test
class, `npx abaplint --fix`, `npm test`, metadata, docs, commit).
...
* `npm run clean` uses `rimraf output` (`rimraf` is a dev dependency) instead of a
  `node -e` one-liner.
* Metadata exists for every class (roadmap orders 1-449). The classes from order
