CLASS ltcl_order_sink_sap DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS rejects_unauthorized FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_mismatched_doc_type FOR TESTING
      RAISING zcx_stock_allocation.
    METHODS rejects_missing_sales_area FOR TESTING
      RAISING zcx_stock_allocation.
ENDCLASS.
...
CLASS ltcl_order_sink_sap IMPLEMENTATION.
  ...
  METHOD rejects_mismatched_doc_type.
    DATA lo_cut TYPE REF TO zif_order_sink.
    DATA lv_raised TYPE abap_bool.
    DATA lv_message TYPE c LENGTH 220.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = lv_message
      exp = 'Sales-order authorization context is invalid' ).
  ENDMETHOD.

  METHOD rejects_missing_sales_area.
    DATA lo_cut TYPE REF TO zif_order_sink.
    DATA lv_raised TYPE abap_bool.
    DATA lv_message TYPE c LENGTH 220.
