...
**Roadmap batch 4 is in progress: orders 344-378 are delivered and verified.**
...
orders 344-378 are delivered and verified.

Next up (in order):

1. 379 `zcl_alloc_mrp` - material requirements planning run.
2. 380 `zcl_alloc_mrp_net` - MRP net requirements.
3. 381 `zcl_alloc_lot_fixed` - lot sizing: fixed lot.
4. 382 `zcl_alloc_lot_period` - lot sizing: period lot.
5. 383 `zcl_alloc_lot_luc` - lot sizing: least unit cost.
6. 384 `zcl_alloc_lot_ppb` - lot sizing: part period balancing.
7. 385 `zcl_alloc_sched_edd` - scheduling: earliest due date.
8. 386 `zcl_alloc_sched_spt` - scheduling: shortest processing time.

Then continue strictly in order 379-443, one feature per iteration, always keeping
`npm test` green.
...
* `npm run clean` uses `rimraf output` (`rimraf` is a dev dependency) instead of a
  `node -e` one-liner.
* Metadata exists for every class (roadmap orders 1-378). The classes from order
