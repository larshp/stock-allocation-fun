CLASS zcl_alloc_key_agg_str DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS zcl_alloc_key_agg_str IMPLEMENTATION.
  ...
  METHOD add.
    ...
    " One append per row, so collecting n rows costs n.
    ls_raw-seq = lines( mt_raw ) + 1.
    ls_raw-key = iv_key.
  ...
  METHOD sums.
    DATA ls_sum  TYPE ty_sum.    DATA lv_key  TYPE string.
    ...
    " One sort by key, with the sequence as tie breaker, then a single forward
    " pass that merges equal keys. Grouping stays linear in the number of rows
    " instead of scanning the whole table once per row.
    IF mv_sorted = abap_false.
      SORT mt_raw BY key seq.
      mv_sorted = abap_true.
    ENDIF.
    ...
    " Restore the order in which the keys first appeared, so a caller that relies
    " on it sees exactly the result it saw before.
    SORT mt_sums BY seq.
  ...
  METHOD find.
    ...
    " The merged table is ordered by the sequence, not by the key, so the lookup
    " sorts a copy by key once and then searches it by halving.
    SORT lt_sums BY key.
